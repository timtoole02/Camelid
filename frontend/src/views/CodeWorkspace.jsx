...
function formatTimingBits(timing) {
  return [
    ...
    Number.isFinite(timing.prefill_ms) ? `${timing.prefill_ms}ms prefill` : null,
    Number.isFinite(timing.reused_tokens) && timing.reused_tokens > 0 ? `${timing.reused_tokens} reused` : null,
    Number.isFinite(timing.prefilled_tokens) && timing.prefilled_tokens > 0 ? `${timing.prefilled_tokens} prefilled` : null,
    timing.engine_rebuilt === true ? `${timing.resident_backend || 'resident'} cold build` : null,
    timing.engine_rebuilt === false && timing.resident_backend ? `${timing.resident_backend} warm` : null,
    typeof timing.batched_prefill === 'boolean' ? (timing.batched_prefill ? 'batched prefill' : 'serial prefill') : null,
  ].filter(Boolean)
}
...
// retained feed advances independently, but avoiding needless reconciliation
// keeps long live transcripts responsive on Windows WebView2.
      ...
      <section className={`ci-group ${planSteps.length ? 'ci-group--joined' : ''}`}>
        <div className="ci-group__head">
          <h3 className="ci-group__label">
            {running ? 'Running' : 'Primary agent'} <span className="ci-num">{(main ? 1 : 0) + runningChildren.length}</span>
          </h3>
        </div>
...
export default function CodeWorkspace({
  ...
  // render per token. The server feed no longer blocks decode on this consumer,
  // but WebView2 still pays for each dispatch and layout. Deltas coalesce
  ...
  // render. Streamed deltas re-render this component on every token; an
  // unmemoized scan here turns a long Windows transcript into avoidable UI jank.
  ...
  // evicts, and even a bounded Code turn can outgrow the display ring. Inferring
  ...
  const openEventStream = (created, resume = false) => {
    ...
    // With no cursor, omit `after` entirely so the server starts at this turn's
    // `turn_start`. Sending `after=0` would replay older retained turns and can
    // deliver their `session.finished` into this live follow-up. The browser's
    // own reconnect adds Last-Event-ID, which the server prefers.
    const after = lastSequenceRef.current
    const eventPath = `/${encodeURIComponent(created.id)}/events${after ? `?after=${after}` : ''}`
    const source = new EventSource(
      workspaceEndpoint(apiBase, eventPath),
    )
  ...
  // Recover only this session's recorded outcome. `/activity` is global to the
  // runtime slot, so a reset/new session may replace it while this detached
  // follower is waiting; the saved thread is the durable fallback.
  const readRecordedOutcome = async (created) => {
    try {
      const snapshot = await getWorkspaceActivity(apiBase)
      const outcome = recordedWorkspaceOutcome(created.id, snapshot, null)
      if (outcome) return String(outcome)
    } catch {}
    try {
      const restored = await getWorkspaceThread(apiBase, created.workspace || workspacePath, created.id)
      const outcome = recordedWorkspaceOutcome(created.id, null, restored)
  ...
  const followDetachedSession = async (created) => {
    ...
    try {
      const terminal = await waitForWorkspaceSessionTerminal(apiBase, created.id, {
        timeoutMs: DETACHED_FOLLOW_TIMEOUT_MS,
        pollMs: DETACHED_FOLLOW_POLL_MS,
      })
      terminalHandledRef.current = true
      const outcome = await readRecordedOutcome(created)
      if (outcome) {
        dispatch({ event: 'session.finished', outcome })
      } else if (terminal?.state === 'cancelled' || terminal?.state === 'failed') {
        dispatch({ event: 'session.finished', outcome: terminal.state })
      } else {
        dispatch({ event: 'session.error', message: 'The coding turn ended, but its final outcome could not be recovered. Refresh Code history to inspect the saved result.' })
      }
    } catch (error) {
      terminalHandledRef.current = true
