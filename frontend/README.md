...
- **Llama 3.2 1B Instruct Q8_0:** exact-row API/WebUI smoke plus compact/broader parity, the supported metadata-Jinja row-template path for the recognized Llama 3 template shape, and bounded 512/1024/2048/4096 context-pack evidence are represented as supported exact-row evidence; the 2048 pack is exact-row only after the RoPE frequency-factor fix, the 4096 pack stays tied to its cited source/runtime head, and the prior 8192 pack is blocked pending a clean rerun after the 2026-05-31 local fallback timeout.
