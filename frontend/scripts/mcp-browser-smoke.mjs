...
try {
  ...
  await clickText('button', 'Add connection')
  ...
  await page.waitForSelector('.mcp-picker summary')
  await page.click('.mcp-picker summary')
  await page.waitForSelector('.mcp-tool-options input:not([disabled])', { timeout: 20000 })
  await page.click('.mcp-tool-options input')
  ...
  console.log('MCP browser smoke passed: connections, selection, approval, actual tool history continuation, and mobile layout.')
} catch (error) {
