...
assert.match(systemSource, /selectedCompatibilityHint\s*=\s*selectedChatGate\.hint\s*\|\|\s*findCompatibilityHint\(capabilities, selectedModel\)/, 'System view selected-row evidence must come from the same exact-row chat gate used for runtime readiness')
...
assert.match(systemSource, /exactRowSupportLanes\(target, apiFeatures\)\.map\(\(lane\) => `\$\{supportLaneTitle\(lane\)\.replace\(' readiness', ''\)\}: \$\{lane\.label\}`\)\.join\(' · '\)/, 'System compatibility rows must render 3B template/context/throughput lanes from /api/capabilities')
