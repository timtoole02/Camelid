...
const spoofedThreeBCatalogFilenameWrongArtifact = {
  ...spoofedThreeBNameWrongArtifact,
  id: 'local-wrong-artifact-with-catalog-filename',
  runtime_model_name: 'local-wrong-artifact-with-catalog-filename',
  hf_filename: 'Llama-3.2-3B-Instruct-Q8_0.gguf',
}
assert.equal(compatibilityHintLabel(findCompatibilityHint(capabilities, spoofedThreeBCatalogFilenameWrongArtifact)), 'llama32_3b_instruct_q8_0: exact GGUF not verified', '3B catalog filename metadata must not override the actual local GGUF filename used by runtime gating')
assert.equal(isCompatibilitySupportedForModel(capabilities, spoofedThreeBCatalogFilenameWrongArtifact), false, '3B support must fail closed when catalog filename metadata is exact but model_path names a neighboring artifact')
