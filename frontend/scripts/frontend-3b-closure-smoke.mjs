...
assert.match(chatSource, /label:\s*compatibilityHintLabel\(selectedChatGate\.hint, 'Exact row unavailable'\)/, 'blocked live 3B capability pills must surface the exact row/quant/artifact blocker instead of generic unavailable copy')
assert.match(chatSource, /copy:\s*selectedChatGate\.hint[\s\S]*\?\s*compatibilityHintCopy\(selectedChatGate\.hint\)/, 'blocked live 3B capability pills must reuse the exact support-contract blocker copy')
assert.match(chatSource, /label:\s*`\$\{selectedChatGate\.hint\.target\.id\} · \$\{template\?\.ready \? 'Template ready' : 'Template gated'\}/, 'live 3B capability label must name the exact /api/capabilities row when lanes are visible')
assert.match(chatSource, /Row-scoped \/api\/capabilities evidence for \$\{selectedChatGate\.hint\.target\.id\}; it does not widen model-native context/, 'live 3B capability copy must not widen support beyond the exact row')
