...
assert.match(chatSource, /models\.map\(\(model\) => \([\s\S]*<option key=\{model\.id\} value=\{model\.id\}>/, 'chat model picker may list known models so users can inspect unavailable selections')
...
assert.match(chatSource, /renderReadinessPills[\s\S]*runtimeStatusLabel[\s\S]*supportStatusLabel[\s\S]*capabilityLaneStatus\.label/, 'non-empty live 3B chat must keep runtime, exact-row support, and row-scoped capability readiness visible after messages exist')
...
assert.match(modelsSource, /selectedExactTarget\s*=\s*isExactCompatibilityHint\(selectedChatGate\.hint\) \? selectedChatGate\.hint\.target : null/, 'ModelsView next-chat state must derive selected exact-row lanes from the shared chat gate')
assert.match(modelsSource, /Selected next-chat exact-row readiness/, 'ModelsView next-chat card must surface the selected exact-row runtime/support/capability gates')
assert.match(modelsSource, /Capability lanes[\s\S]*selectedSupportLanes\.length[\s\S]*supportLaneTitle\(lane\)[\s\S]*lane\.label/, 'ModelsView next-chat card must render row-scoped capability lanes instead of a generic ready state')
