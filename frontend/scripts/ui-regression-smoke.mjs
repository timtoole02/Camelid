...
assert.match(chatWorkspaceSource, /apiUnavailable\s*=\s*runtime\?\.status === 'offline'/, 'Chat landing should detect API-unavailable state instead of treating it as only a missing model')
assert.match(chatWorkspaceSource, /Connect Camelid\./, 'Chat landing should show a direct API-unavailable title when Camelid cannot be reached')
assert.match(chatWorkspaceSource, /Chat is unavailable until Camelid responds/, 'Composer readiness note should explain API-unavailable state without unsupported model claims')
assert.match(chatWorkspaceSource, /setTab\(apiUnavailable \? 'api' : 'library'\)/, 'API-unavailable chat actions should lead to API settings instead of only Models')
assert.match(chatWorkspaceSource, /\{apiUnavailable \? 'Open API' : 'Add model'\}/, 'No-model action should become API settings when the API is unavailable')
assert.match(chatWorkspaceSource, /!selectedModelRunnable && models\.length > 0 && <button/, 'Composer should not render duplicate setup buttons when no models are available')
assert.match(topBarSource, /apiUnavailable\s*=\s*runtime\?\.status === 'offline'/, 'TopBar should distinguish API offline from model-not-ready state')
assert.match(topBarSource, /\? 'Offline'/, 'TopBar chat readiness label should surface API offline state')
