...
assert.match(messageTurnSource, /title="Decode output rate after the first token, measured in this browser"/, 'the browser decode metric must state that prompt prefill and TTFT are excluded')
...
assert.match(diagnosticsSource, /Decode Output Rate/, 'developer diagnostics must use the same truthful browser-rate label')
...
assert.match(dashboardHookSource, /metrics\.elapsedMs - metrics\.firstContentMs[\s\S]*tokensPerSecond\(realTokens - 1, decodeElapsedMs\)/, 'live decode rate must use the stream-relative post-first-token interval, excluding prompt prefill without mixing clock origins')
