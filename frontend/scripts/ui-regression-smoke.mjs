...
assert.match(topBarSource, /exactSupportedTargetFromHint\(activeChatGate\.hint\)[\s\S]*exactSupportedTargetFromHint\(selectedChatGate\.hint\)[\s\S]*getCurrentCompatibilityTarget/, 'TopBar support contract detail should prefer active or selected exact-row evidence before the first current-gate fallback')
