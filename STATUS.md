...
Executive summary: Camelid has API + frontend exact-row validation for the Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Q8_0 rows within their checked envelopes, with TinyLlama Q8_0 still the baseline gate. Llama 3.2 1B retains bounded 512/1024/2048/4096-context packs where row-specific PASS bundles are cited; its previously cited 8192 pack is blocked pending a clean rerun after the 2026-05-31 local fallback timeout. Llama 3.2 3B is supported only as exact-row smoke, with canonical Ubuntu API/WebUI refresh at source head `e9f926ed1a65` plus bounded 512/1024/2048-context packs where row-specific PASS bundles are cited; Llama 3 8B is promoted through exact-row smoke plus checked bounded 512/1024/2048-context packs where row-specific PASS artifacts are cited. Mistral 7B Instruct v0.3 has evidence-only bring-up with checked tokenizer/template, parity, and bounded 512/1024/2048/4096/8192 context artifacts, but the support contract remains fail-closed until API/WebUI/RSS readiness is synchronized. Mixtral-8x7B-Instruct-v0.1 Q8_0 has bounded one-token backend MoE runtime evidence only; Gate 9A later-generation evidence diverges and the continuation lane recorded a backend HTTP hang, so no Mixtral API/WebUI/frontend readiness or broad support claim is active. The public support boundary moves only for supported exact rows and only for validated local-chat/parity envelopes; broad-family support, model-native/larger contexts beyond checked packs, arbitrary template execution beyond row-scoped renderer/template evidence, production throughput, and portability remain outside the support claim.
...
| Llama 3.2 1B Instruct Q8_0 | Verified bounded support moved beyond a demo: API, WebUI, compact/broader parity, exact-row metadata-Jinja row-template parity, bounded template-shapes, unique-chat RSS/perf, and post-fix 2048 evidence all agree. | 512, 1024, 2048, and 4096 bounded packs are green; 2048 is green only after the RoPE frequency-factor fix, 4096 is green on source/runtime head `470388f8165b`, and the previously cited 8192 pack on source/runtime head `aaf9207d1669` is now blocked pending a clean rerun after the 2026-05-31 local fallback timeout. | Broader/full support still needs model-native/larger context, the blocked 8192 rerun, broader arbitrary-template coverage beyond the supported 1B metadata-Jinja row template, production throughput, portability, and durable full-support normalization. |
...
| Mistral-7B-Instruct-v0.3.Q8_0.gguf | Evidence-only bring-up: exact-row tokenizer/template, 1-token generation, broader 50-token parity, and bounded context evidence exist, but API/WebUI/RSS support-surface proof is fail-closed. | 512, 1024, 2048, 4096, and 8192 bounded packs are green for this exact row only. | Support/readiness promotion, Mistral-family support, neighboring Mistral variants, arbitrary templates beyond the row-scoped renderer and shapes, model-native/larger context beyond the checked packs, or production throughput. |
...
This is the repo story in one sentence: Camelid preserves one trusted small-model gate and three bounded Llama exact-row lanes, while keeping Mistral/Mixtral/Qwen/Gemma fail-closed for every unproven support, readiness, context, quantization, and runtime path.
...
Public readiness language is locked to row-by-row evidence: the Mistral row may say only “evidence-only bring-up; API/WebUI/RSS support contract fail-closed”; the Mixtral row may say only “bounded one-token backend MoE runtime evidence; later-generation/API/WebUI/frontend readiness blocked”; Qwen and Gemma may say “planned exact-row candidate, not supported yet.” Any Mistral or Mixtral support/readiness claim requires row-specific API/WebUI/RSS/frontend evidence that is green and synchronized with `/api/capabilities`.
...
- `qa/evidence-bundles/llama32-1b-context-8192-current-head-20260513T183501Z-head-aaf9207d1669/manifest.json` records the historical fifth bounded 8192-context compact-template pack pass for the exact Llama 3.2 1B Instruct Q8_0 row on source/runtime head `aaf9207d1669`: the 7650-token prompt matched prompt tokens, generated token IDs `[34,2735,35,12,18831]`, and generated text `CMLD-819` for the checked 5-token pack. Current status is blocked pending clean rerun: `qa/warroom/2026-05-31/recon/2026-05-31T1545-correctness-lead-1b-8192-blocker.md` records a local fallback timeout after 120000 ms before Camelid diagnostics. Do not promote the 8192 bucket from current API/WebUI/docs until that conflict is resolved.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 512/1024/2048/4096/8192 context fields and promotes only the exact buckets with row-specific current evidence: Llama 3.2 1B through 4096 with 8192 marked blocked pending rerun, and Llama 3.2 3B plus Llama 3 8B through 2048. These remain exact bounded-pack claims only.
...
- The fourth bounded 4096-context pack is green for this exact 1B row: 4096 matched `CMLD-409` on source/runtime head `470388f8165b`. The prior fifth bounded 8192-context pack matched `CMLD-819` on source/runtime head `aaf9207d1669` with 7650 prompt tokens and max timed-process RSS `3806876 KiB`, but the latest local fallback rerun timed out after 120000 ms, so 8192 is blocked pending clean rerun.
...
- The support claim is limited to this exact 1B Instruct Q8_0 row, short local-chat smoke, the checked first bounded 512-context pack, the checked second bounded 1024-context pack, the checked third bounded 2048-context pack, the checked fourth bounded 4096-context pack, the checked compact chat-template-shapes pack, the exact-row metadata-Jinja row-template path, and the bounded unique-chat perf/RSS envelope; the 8192 pack is blocked pending clean rerun. Neighboring Llama rows, other quantizations, model-native/larger contexts beyond checked packs, broader arbitrary GGUF/Jinja template execution, broad chat-template behavior, production throughput, and portability remain outside the claim.
...
6. Preserve Mistral-7B-Instruct-v0.3.Q8_0.gguf as evidence-only bring-up across all surfaces until support-surface proof is synchronized.
...
Mistral 7B Instruct v0.3 Q8_0 remains evidence-only bring-up. The GGUF has exact-row tokenizer/template, 1-token generation, broader 50-token parity, and checked 512/1024/2048/4096/8192 context packs, but support/readiness promotion is blocked until a scrubbed support-surface bundle shows synchronized API/WebUI/RSS readiness and `/api/capabilities` reports the same contract. Broad Mistral-family quants/sizes, larger native contexts, production throughput, portability, and neighboring rows remain unsupported pending further row-specific evidence.
