...
| Llama 3.2 3B Instruct Q8_0 | The previous broader prompt-pack blocker is closed, API/WebUI are green, exact-row metadata-Jinja template execution is now enabled, 2048 is green, and the opt-in parallel Q8 first-token direction probe gives a concrete performance lead. | 512, 1024, and 2048 bounded packs are green for this exact row. | Broader/full support still needs model-native/larger context, production throughput beyond the direction probe, portability, and durable full-support normalization. |
