...
| Mistral-7B-Instruct-v0.3.Q8_0.gguf | Active validation only: exact-row tokenizer/template, 1-token generation, broader 50-token parity, bounded contexts, and fail-closed API/WebUI/RSS evidence exist. | 512, 1024, 2048, 4096, and 8192 bounded packs are green for this exact row only. | Support-contract promotion, WebUI chat enablement, Mistral-family support, neighboring variants, arbitrary templates beyond row-scoped renderer/shapes, model-native/larger context beyond checked packs, or production throughput. |
...
This is the repo story in one sentence: Camelid preserves one trusted small-model gate and three bounded Llama exact-row lanes, while keeping Mistral/Mixtral/Qwen/Gemma fail-closed for every unproven support, context, quantization, and runtime path.
...
Public readiness language is locked to row-by-row evidence: the Mistral row may say only “active validation; tokenizer/template, 1-token, broader 50-token, bounded-context, and fail-closed API/WebUI/RSS evidence exist, but support/WebUI readiness is blocked”; the Mixtral row may say only “bounded one-token backend MoE runtime evidence; later-generation/API/WebUI/frontend readiness blocked”; Qwen and Gemma may say “planned exact-row candidate, not supported yet.” Any Mistral or Mixtral support/readiness claim requires row-specific API/WebUI/RSS/frontend evidence with the support contract actually promoted.
...
6. Preserve the Mistral-7B-Instruct-v0.3.Q8_0.gguf exact-row validation evidence while keeping support and WebUI readiness fail-closed until a promoted support-contract bundle exists.
...
Mistral 7B Instruct v0.3 Q8_0 remains active validation only. The GGUF has exact-row tokenizer/template, 1-token generation, broader 50-token parity, checked 512/1024/2048/4096/8192 context packs, and a current-head API/WebUI/RSS fail-closed bundle. That bundle expected `active_validation_unsupported`, `contract_supported=false`, and `webui_chat=blocked`, so it is readiness guard evidence, not support-promotion evidence. Next promotable boxes are explicit support-contract promotion, WebUI chat enablement, broad Mistral-family quants/sizes, larger native contexts, or production throughput, all pending further row-specific evidence.
