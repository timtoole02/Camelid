...
Last updated: 2026-05-31 20:15 PDT / 2026-06-01 03:15 UTC

Branch: `release/v0.1-evidence`

Current release branch head SHA before this status update: `8840b0186e9e266915f5b33e2a6202dfc3bd50cb`
...
- Confirmed local branch head and `origin/release/v0.1-evidence` at `8840b0186e9e266915f5b33e2a6202dfc3bd50cb` before this status-only refresh.
- Fetched from origin; `origin/release/v0.1-evidence` remained unchanged.
- Re-recorded the active release blocker with current branch, remote, tag, hosted-CI, and local-dispatch evidence.
- Ran a fresh full local branch-head gate refresh on the clean release worktree.
...
- Status observation timestamp: 2026-05-31 20:15 PDT / 2026-06-01 03:15 UTC.
- `git status --short --branch` returned `## release/v0.1-evidence...origin/release/v0.1-evidence` with no file changes.
- Branch/remote observation: `release/v0.1-evidence` and `origin/release/v0.1-evidence` both resolved to `8840b0186e9e266915f5b33e2a6202dfc3bd50cb`.
- Recent release-history observation: `8840b01 chore(release): refresh release decision blocker`; `e1978da chore(release): refresh rc1 blocker observation`; `578757d chore(release): record current rc1 blocker`; `52eda90 chore(release): clarify validated gate sha`; `4248f55 chore(release): refresh branch-head gate observation`.
...
- `git describe --tags --exact-match HEAD` failed with `fatal: no tag exactly matches '8840b0186e9e266915f5b33e2a6202dfc3bd50cb'`, confirming the current release branch head is not tagged.
...
- GitHub connector observation: workflow runs and combined commit statuses were empty for both current branch head `8840b0186e9e266915f5b33e2a6202dfc3bd50cb` and rc1 tag target `d9fb294f47e3ae80291f969499e2240c6cd640c3`.
- Local `gh` observation: `command -v gh` returned no path, so local workflow dispatch remains unavailable from this environment.
- Local QA refresh passed at `8840b0186e9e266915f5b33e2a6202dfc3bd50cb`: `cargo fmt --all -- --check`; `cargo clippy --all-targets --all-features -- -D warnings`; `cargo check --all-targets --all-features`; `cargo test --all-targets --all-features --no-fail-fast`; `cargo build --release --bin camelid`; `cargo doc --no-deps --all-features`; frontend `npm ci`, build, model-state smoke, 3B closure smoke, integration smoke, streaming smoke, and UI smoke; all `scripts/test-*.mjs`; benchmark harness self-test; public evidence-claim check; public scrub guard; README screenshot guard; v0.1 privacy audit; evidence checksum check; and `git diff --check`.
...
- Public evidence claims checked 97 manifests and 49 summary files; v0.1 privacy audit found zero findings; evidence checksum verification passed.
...
- Commits after `5017ff28568dc1898fa490b4492848a1b3c022f0` through `8840b0186e9e266915f5b33e2a6202dfc3bd50cb` are release documentation/status updates to `RELEASE_GATE_v0.1.md`, `RELEASE_REPORT_v0.1.md`, and `RELEASE_STATUS.md`.
...
- The existing `v0.1.0-rc1` tag target predates the frontend/README QA repair at `3001fa48e3d5fa41dbeca417dd511164a3bacc80` and the current status-only branch head at `8840b0186e9e266915f5b33e2a6202dfc3bd50cb`. The tag was not moved; Tim needs to decide whether to cut a new candidate, retag, or keep rc1 as a known-pre-fix candidate.
...
- No local gate is currently recorded failing at current branch head `8840b0186e9e266915f5b33e2a6202dfc3bd50cb`; this update is documentation-only.
...
- Decide whether the repaired release branch, including validated pre-doc-refresh head `5017ff28568dc1898fa490b4492848a1b3c022f0`, current status-only head `8840b0186e9e266915f5b33e2a6202dfc3bd50cb`, and this status-only follow-up, should become a new release candidate, a retagged rc1, or remain only on the release branch. No final `v0.1.0` tag is authorized by this release-captain update.
