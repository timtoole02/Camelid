...
try {
  ...
  assert.ok(plan.method.bounded_metrics.some((metric) => metric.includes('top projection routes')))
  assert.ok(plan.method.bounded_metrics.some((metric) => metric.includes('FFN-down decode')))
  assert.ok(plan.outputs.stdout.some((line) => line.includes('camelid_backend_q8_calls')))
