...
fn navigate_engine_ui(app: &tauri::AppHandle, url: &str) {
    const LOAD_TITLE: &str = "Engine UI could not load";
    const LOAD_GUIDANCE: &str =
    ...
    let target = match serde_json::to_string(url) {
        ...
        Err(e) => {
            emit_error(
                ...
                LOAD_TITLE,
                LOAD_GUIDANCE,
                &format!("could not encode the engine URL: {e}"),
            );
            return;
        }
    };
    if let Err(e) = window.eval(format!("window.location.replace({target})")) {
        emit_error(
            ...
            LOAD_TITLE,
            LOAD_GUIDANCE,
            &format!("could not retry loading the engine UI: {e}"),
        );
        return;
    }
    if !wait_for_navigation(&window, &parsed, Duration::from_secs(5)) {
