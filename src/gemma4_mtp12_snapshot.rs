...
fn packed_prefix(
    ...
    for head in 0..heads {
        ...
        for value in &values[base..base + prefix * dim] {
            if !value.is_finite() {
                return Err(diagnostic_error("non-finite committed KV"));
            }
            bytes.extend_from_slice(&value.to_le_bytes());
        }
    }
...
impl Gemma4GpuRuntime {
    pub(super) fn maybe_dump_mtp12_final_kv(
        ...
        generated: &[u32],
        position: usize,
        stats: &Gemma4Mtp12MetalStats,
    ) -> Result<()> {
        let Some(root) = enabled_path() else {
            return Ok(());
        };
        ...
        self.model
            .with_kv_device_views(&[46, 47], |views| -> Result<()> {
                for view in views {
                    let expected = if view.source_layer == 46 {
                        (8, 256)
                    } else {
                        (1, 512)
                    };
                    ...
                    for (kind, buffer) in [("key", view.key), ("value", view.value)] {
                        ...
                        files.push(
                            serde_json::json!({"file":name,"layer":view.source_layer,"kind":kind,
        ...
        let environment = std::env::vars()
            .filter(|(k, _)| {
                k.starts_with("CAMELID_GEMMA4_")
                    || k.starts_with("CAMELID_MTP12_")
                    || matches!(
                        k.as_str(),
                        "CAMELID_SKIP_FIT_CHECK" | "CAMELID_CONTEXT_PAGING"
                    )
            })
        ...
        std::fs::write(
            ...
            serde_json::to_vec_pretty(&metadata).map_err(diagnostic_error)?,
        )
        .map_err(diagnostic_error)?;
        eprintln!(
            "[gemma4-mtp12] final committed KV snapshot: {}",
            directory.display()
        );
        Ok(())
    }
