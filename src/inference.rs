...
pub(super) const X86_Q8_PACKED_ROWS4_MATMUL_GROUPS_PER_CHUNK: usize = 8;
...
fn try_x86_q8_output_packed_rows4_matmul_path(
    ...
    q8_0_packed_rows4_matmul_projection(input, packed, output_width, name, runtime_plan).map(Some)
}
...
fn q8_packed_rows4_matmul_parallel_chunk_floats(
    ...
    let groups_per_chunk = total_output_groups.clamp(1, groups_per_chunk);
    groups_per_chunk * 4
}
...
fn q8_0_packed_rows4_matmul_projection_from_quantized(
    ...
    if should_parallelize_q8_packed_rows4_matmul(total_output_groups) {
        let chunk_floats =
            q8_packed_rows4_matmul_parallel_chunk_floats(total_output_groups, groups_per_chunk);
        output
            .par_chunks_mut(chunk_floats)
            .enumerate()
...
fn q8_0_packed_rows4_gemm4_projection_with_row_group_schedule(
    ...
    if packed_rows == 0 {
        ...
        return q8_0_packed_rows4_matmul_projection(
            input,
            packed,
            output_width,
            name,
...
fn q8_0_packed_rows4_matmul_projection_pair_from_quantized(
    ...
    {
        let chunk_floats = q8_packed_rows4_matmul_parallel_chunk_floats(
            total_left_output_groups,
            groups_per_chunk,
        );
        left_output
            .par_chunks_mut(chunk_floats)
...
fn q8_0_packed_rows4_matmul_projection_triplet_from_quantized(
    ...
    {
        let chunk_floats =
            q8_packed_rows4_matmul_parallel_chunk_floats(total_q_output_groups, groups_per_chunk);
        q_output
            .par_chunks_mut(chunk_floats)
            .zip(k_output.par_chunks_mut(chunk_floats))
            .zip(v_output.par_chunks_mut(chunk_floats))
