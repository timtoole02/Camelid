...
mod ghost_moe_cli_tests {
    ...
    #[test]
    fn video_generate_parses_minimax_h3_safe_defaults() {
        on_cli_test_stack(|| {
            let cli = Cli::try_parse_from([
                "camelid",
                "video",
                "generate",
                "--prompt",
                "a red panda walking through mist",
                "--dry-run",
            ])
            .expect("parse MiniMax-H3 video command");
            match cli.command {
                Some(Command::Video {
                    action: VideoAction::Generate(args),
                }) => {
                    assert_eq!(args.variant, camelid::minimax_h3::H3Variant::Fl2va);
                    assert_eq!(
                        (args.width, args.height, args.frames, args.steps),
                        (640, 384, 25, 4)
                    );
                    assert!(!args.no_offload_to_cpu);
                    assert!(args.dry_run);
                }
                other => panic!("expected Video Generate, got {other:?}"),
            }
        });
    }

    #[test]
    fn video_pull_requires_an_explicit_license_acknowledgement_flag() {
        on_cli_test_stack(|| {
            let cli = Cli::try_parse_from(["camelid", "video", "pull"])
                .expect("the runtime, not clap, reports the missing acknowledgement");
            match cli.command {
                Some(Command::Video {
                    action: VideoAction::Pull { accept_license, .. },
                }) => assert!(!accept_license),
                other => panic!("expected Video Pull, got {other:?}"),
            }
        });
    }
}
...
struct VideoGenerateArgs {
    /// H3 checkpoint: fl2va for text/first/last frames, ref2va for references.
    #[arg(long, value_enum, default_value_t = camelid::minimax_h3::H3Variant::Fl2va)]
    variant: camelid::minimax_h3::H3Variant,
    #[arg(
        long,
        default_value = "models/minimax-h3",
        env = "CAMELID_H3_MODELS_DIR"
    )]
    models_dir: PathBuf,
    /// Override the selected variant's diffusion GGUF.
    #[arg(long)]
    diffusion_model: Option<PathBuf>,
    /// Override the MiniMax-H3 Qwen3-VL-32B text-encoder GGUF.
    #[arg(long)]
    text_encoder: Option<PathBuf>,
    /// Override minimax_h3_video_vae_fp16.safetensors.
    #[arg(long)]
    video_vae: Option<PathBuf>,
    /// Override minimax_h3_audio_vae_fp32.safetensors.
    #[arg(long, conflicts_with = "no_audio")]
    audio_vae: Option<PathBuf>,
    /// Generate silent video without loading the audio VAE.
    #[arg(long, default_value_t = false)]
    no_audio: bool,
    /// Path to a current stable-diffusion.cpp sd-cli binary.
    #[arg(long, env = "CAMELID_SD_CLI")]
    sd_cli: Option<PathBuf>,
    /// Video and audio description.
    #[arg(long, short = 'p')]
    prompt: String,
    /// Output path (.webm recommended; .avi also supported).
    #[arg(long, short = 'o', default_value = "output.webm")]
    output: PathBuf,
    #[arg(long, default_value_t = 640)]
    width: u32,
    #[arg(long, default_value_t = 384)]
    height: u32,
    /// Requested frames at 24 fps; H3 rounds up to its 17k+5 temporal grid.
    #[arg(long, default_value_t = 25)]
    frames: u32,
    ...
    seed: i64,
    /// Initial frame (fl2va only).
    #[arg(long)]
    init_image: Option<PathBuf>,
    /// Final frame; requires --init-image (fl2va only).
    #[arg(long)]
    end_image: Option<PathBuf>,
    /// Reference image; repeat as needed (ref2va only).
    #[arg(long = "reference-image")]
    reference_images: Vec<PathBuf>,
    /// Directory of lexicographically sorted reference frames; repeat as needed.
    #[arg(long = "reference-video")]
    reference_videos: Vec<PathBuf>,
    /// Reference WAV file; repeat as needed (ref2va only).
    #[arg(long = "reference-audio")]
    reference_audios: Vec<PathBuf>,
    /// Keep all possible weights resident instead of using the safer CPU-offload default.
    #[arg(long, default_value_t = false)]
    no_offload_to_cpu: bool,
    /// Validate and print the exact backend invocation without running it.
    #[arg(long, default_value_t = false)]
    dry_run: bool,
}
...
#[derive(Debug, Subcommand)]
enum VideoAction {
    /// Download and SHA-256 verify the selected MiniMax-H3 bundle (~25 GiB).
    Pull {
        /// H3 checkpoint: fl2va for text/first/last frames, ref2va for references.
        #[arg(long, value_enum, default_value_t = camelid::minimax_h3::H3Variant::Fl2va)]
        variant: camelid::minimax_h3::H3Variant,
        /// Bundle directory (kept separate from autoregressive chat GGUFs).
        #[arg(
            long,
            default_value = "models/minimax-h3",
            env = "CAMELID_H3_MODELS_DIR"
        ...
        models_dir: PathBuf,
        /// Skip the audio VAE and emit silent video.
        #[arg(long, default_value_t = false)]
        no_audio: bool,
        /// Confirm that you read and accept the MiniMax-H3 Community License.
        #[arg(long, default_value_t = false)]
        accept_license: bool,
    },
    /// Check the H3 artifacts and stable-diffusion.cpp backend without generating.
    Doctor {
        #[arg(long, value_enum, default_value_t = camelid::minimax_h3::H3Variant::Fl2va)]
        variant: camelid::minimax_h3::H3Variant,
        #[arg(
        ...
        #[arg(long, env = "CAMELID_SD_CLI")]
        sd_cli: Option<PathBuf>,
        /// Skip the audio VAE requirement and report a silent-video bundle.
        #[arg(long, default_value_t = false)]
        no_audio: bool,
        /// Hash every artifact (reads roughly 25 GiB for the default bundle).
        #[arg(long, default_value_t = false)]
        verify_sha256: bool,
    },
    /// Generate a local WebM/AVI clip with MiniMax-H3 via stable-diffusion.cpp.
    Generate(Box<VideoGenerateArgs>),
}
...
enum Command {
    ...
    /// Download, diagnose, and generate video with the experimental MiniMax-H3 lane.
    Video {
        #[command(subcommand)]
        action: VideoAction,
    },
    ...
    match command {
        ...
        Command::Video { action } => match action {
            VideoAction::Pull {
                ...
                camelid::minimax_h3::pull_bundle(&models_dir, variant, !no_audio, accept_license)?;
                eprintln!(
                    "\nMiniMax-H3 {:?} bundle is ready in {}",
                    variant,
                    models_dir.display()
                );
                eprintln!(
                    "Check the backend with: camelid video doctor --variant {} --models-dir {}",
                    match variant {
                        camelid::minimax_h3::H3Variant::Fl2va => "fl2va",
                        camelid::minimax_h3::H3Variant::Ref2va => "ref2va",
                    },
                    models_dir.display()
                );
            }
            VideoAction::Doctor {
                ...
                let sd_cli = camelid::minimax_h3::resolve_sd_cli(sd_cli.as_deref());
                let report = camelid::minimax_h3::doctor(
                    &models_dir,
                    variant,
                    !no_audio,
                    &sd_cli,
                ...
                anyhow::ensure!(
                    report.backend_ready && report.artifacts_ready,
                    "MiniMax-H3 doctor found missing or incompatible requirements"
                );
            }
            VideoAction::Generate(args) => {
                let VideoGenerateArgs {
                    variant,
                    models_dir,
                    diffusion_model,
                    text_encoder,
                ...
                let mut bundle =
                    camelid::minimax_h3::H3Bundle::from_dir(&models_dir, variant, !no_audio);
                if let Some(path) = diffusion_model {
                    bundle.diffusion_model = path;
                }
                if let Some(path) = text_encoder {
                    bundle.text_encoder = path;
                }
                if let Some(path) = video_vae {
                    bundle.video_vae = path;
                }
                if let Some(path) = audio_vae {
                    bundle.audio_vae = Some(path);
                }
                let request = camelid::minimax_h3::GenerateRequest {
                    prompt,
                    output,
                    width,
                    height,
                    ...
                    reference_videos,
                    reference_audios,
                    offload_to_cpu: !no_offload_to_cpu,
                };
                let sd_cli = camelid::minimax_h3::resolve_sd_cli(sd_cli.as_deref());
                let plan = camelid::minimax_h3::generate(&sd_cli, &bundle, &request, dry_run)?;
                println!("{}", serde_json::to_string_pretty(&plan)?);
                if !dry_run {
                    eprintln!("MiniMax-H3 video ready at {}", request.output.display());
                }
            }
        },
