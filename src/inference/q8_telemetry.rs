...
use super::{
    env_flag_enabled, q8_0_selected_borrowed_packed_rows4, BorrowedLinearWeight, Q8_0_BLOCK_VALUES,
};
...
#[allow(dead_code)]
pub(super) fn record_q8_schedule_activation_pack<T>(
    packed_inputs: &mut Vec<T>,
    ...
    let requested_bytes = requested_blocks.saturating_mul(mem::size_of::<T>());
    let before_capacity_bytes = before_capacity.saturating_mul(mem::size_of::<T>());
    let after_capacity_bytes = packed_inputs.capacity().saturating_mul(mem::size_of::<T>());
