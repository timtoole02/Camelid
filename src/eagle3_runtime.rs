...
use crate::eagle3::{Eagle3DraftModel, Eagle3Geometry, TARGET_LAYER_INPUT_IDS, TARGET_VOCAB_SIZE};
...
use crate::metal::{
    ...
    ResidentIndexedHeadEarlySnapshot, EAGLE3_DRAFT_VOCAB,
    RESIDENT_INDEXED_HEAD_SHADOW_MAX_CANDIDATES,
};
use crate::tensor::CpuTensor;
#[cfg(test)]
use crate::{eagle3::HIDDEN_SIZE, metal::EAGLE3_AUX_WIDTH};
...
pub fn interleave_target_layer_inputs(captures: &[CpuTensor]) -> Result<Vec<f32>> {
    ...
    let hidden = captures[0].dim(1)?;
    if !matches!(hidden, 2560 | 3072) {
        return Err(invalid(format!(
            "unsupported EAGLE-3 capture width {hidden}"
        )));
    }
    let aux_width = 3 * hidden;
    ...
    for (slot, capture) in captures.iter().enumerate() {
        ...
        if capture_rows != rows || width != hidden {
            return Err(invalid(format!(
                "EAGLE-3 target capture {} (layer input {}) has shape {:?}, expected [{rows}, {hidden}]",
                slot, TARGET_LAYER_INPUT_IDS[slot], capture.shape.dims
            )));
        }
    }
    let mut features = vec![0.0f32; rows * aux_width];
    for row in 0..rows {
        for (tap, capture) in captures.iter().enumerate() {
            let source = row * hidden;
            let destination = row * aux_width + tap * hidden;
            features[destination..destination + hidden]
                .copy_from_slice(&capture.data[source..source + hidden]);
        }
    }
    Ok(features)
}
...
impl Eagle3AuthoritativeCatchup {
    ...
    pub fn push(&mut self, captures: &[CpuTensor], emitted: &[u32]) -> Result<()> {
        ...
        let aux_width = 3 * captures[0].dim(1)?;
        if !self.emitted.is_empty()
            && self.interleaved_features.len() / self.emitted.len() != aux_width
        {
            return Err(invalid("EAGLE-3 deferred capture geometry changed"));
        }
        let rows = features.len() / aux_width;
        if emitted.len() > rows {
            return Err(invalid(format!(
...
impl Eagle3DynamicFrontier {
    pub fn new(anchor: u32, config: Eagle3DynamicFrontierConfig) -> Result<Self> {
        Self::with_geometry(anchor, config, Eagle3Geometry::LLAMA)
    }
    fn with_geometry(
        anchor: u32,
        config: Eagle3DynamicFrontierConfig,
        ...
        let target_vocab = geometry.target_vocab;
        let config = config.validate()?;
        if config.certified_argmax_shadow && anchor as usize >= target_vocab {
            return Err(invalid(format!(
                "EAGLE-3 certified-argmax shadow anchor id {anchor} is outside target 0..{target_vocab}"
            )));
        }
        Ok(Self {
            geometry,
    ...
    pub fn record_expansion(
        ...
        let target_vocab = self.geometry.target_vocab;
        ...
        if output.raw_hidden.len() != hidden {
            return Err(invalid(format!(
                "EAGLE-3 frontier output hidden width is {}, expected {hidden}",
        ...
        if self.certified_argmax_shadow.is_some() {
            if output.draft_token as usize >= EAGLE3_DRAFT_VOCAB
                || output.target_token as usize >= target_vocab
            {
                return Err(invalid(format!(
                    "EAGLE-3 certified-argmax shadow top-1 ids draft={} target={} are outside draft 0..{EAGLE3_DRAFT_VOCAB} / target 0..{target_vocab}",
            ...
            for (slot, candidate) in output.top_candidates.iter().enumerate() {
                if candidate.draft_token as usize >= EAGLE3_DRAFT_VOCAB
                    || candidate.target_token as usize >= target_vocab
                {
                    return Err(invalid(format!(
                        "EAGLE-3 certified-argmax shadow candidate {slot} ids draft={} target={} are outside draft 0..{EAGLE3_DRAFT_VOCAB} / target 0..{target_vocab}",
...
#[derive(Debug, Clone, PartialEq, Eq)]
// The receipt is deliberately carried inline: boxing it would change the shape of a
// public promotion outcome for no behavioural gain.
...
impl Eagle3Drafter {
    ...
    fn accept_authoritative_features(
        ...
        let aux_width = self.head.geometry().aux_width();
        if emitted.is_empty() {
            return Err(invalid(
                "an EAGLE-3 verify round must emit at least one token",
            ));
        }
        if !features.len().is_multiple_of(aux_width) {
            return Err(invalid(format!(
                "EAGLE-3 authoritative features have {} values, not a multiple of {aux_width}",
                features.len()
            )));
        }
        let rows = features.len() / aux_width;
        ...
        if eagle3_authoritative_cb_fusion_enabled() {
            ...
            let output = metal(self.head.forward_authoritative_features_last_output_fused(
                &embeddings.data,
                &features[..emitted.len() * aux_width],
                start,
            ))?;
        ...
        let fused = metal(
            self.head
                .fuse_features(&features[..emitted.len() * aux_width]),
        )?;
        let embeddings = target_weights
            .token_embedding
            .embedding_lookup(emitted, "eagle3_authoritative_next_token_embeddings")?;
    ...
    pub fn new(model: &Eagle3DraftModel, max_positions: usize) -> Result<Self> {
        ...
        let head = metal(Eagle3MetalState::new_with_geometry(
            weights,
            max_positions,
        ...
        F: FnMut(Eagle3NextExpansionEvidence) -> bool,
    {
        ...
        let mut frontier =
            Eagle3DynamicFrontier::with_geometry(anchor, config, self.head.geometry())?;
