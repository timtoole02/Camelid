...
pub(super) const API_CONFORMANCE_CASES: &[ApiConformanceCase] = &[
    ...
    ApiConformanceCase {
        ...
        status: SupportStatus::SupportedCurrentGate,
        notes: "temperature is applied on the dense and runnable chat lanes, streaming and non-streaming. Values below 0 are a typed 400; there is no upper bound and high values produce incoherent text. Parity evidence for supported rows is greedy (temperature 0) only — sampled output is outside it. The gemma4 lane ignores sampling and always decodes greedily.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "completions"],
        unsupported_modes: &["gemma4_lane"],
    },
    ApiConformanceCase {
        id: "sampling_top_p",
        status: SupportStatus::SupportedCurrentGate,
        notes: "top_p is applied on the dense and runnable chat lanes, streaming and non-streaming. Must be finite and in (0, 1]; anything else is a typed 400. Only meaningful with temperature above 0. The gemma4 lane ignores sampling and always decodes greedily.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "completions"],
        unsupported_modes: &["gemma4_lane"],
    },
    ApiConformanceCase {
        id: "sampling_top_k",
        status: SupportStatus::SupportedCurrentGate,
        notes: "top_k is applied on the dense and runnable chat lanes, streaming and non-streaming. top_k of 1 reproduces greedy decoding exactly. A negative value fails to deserialize as a typed 400. Only meaningful with temperature above 0. The gemma4 lane ignores sampling and always decodes greedily.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "completions"],
        unsupported_modes: &["gemma4_lane"],
    },
    ApiConformanceCase {
        id: "sampling_min_p",
        status: SupportStatus::SupportedCurrentGate,
        notes: "min_p is applied on the dense and runnable chat lanes, streaming and non-streaming. Must be finite and in [0, 1]; anything else is a typed 400. Only meaningful with temperature above 0. The gemma4 lane ignores sampling and always decodes greedily.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "completions"],
        unsupported_modes: &["gemma4_lane"],
    },
    ApiConformanceCase {
        id: "sampling_seed",
        status: SupportStatus::SupportedCurrentGate,
        notes: "seed makes sampled generation reproducible on the dense and runnable chat lanes: the same seed and request return the same text, and different seeds diverge. Without a seed, sampled decoding is still deterministic per request. A seed has no observable effect at temperature 0, where decoding is greedy. The gemma4 lane ignores sampling and always decodes greedily.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "completions"],
        unsupported_modes: &["gemma4_lane"],
    },
