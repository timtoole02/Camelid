...
pub(super) const API_CONFORMANCE_CASES: &[ApiConformanceCase] = &[
    ...
    ApiConformanceCase {
        ...
        status: SupportStatus::SupportedCurrentGate,
        notes: "temperature is applied on the dense and runnable chat lanes, streaming and non-streaming. On those lanes a value below 0, or a non-finite value, is a typed 400; there is no upper bound and high values produce incoherent text. Parity evidence for supported rows is greedy (temperature 0) only — sampled output is outside it. The gemma4 and diffusion-gemma lanes ignore sampling and always decode greedily, so neither the typed 400s nor the parameter itself applies there.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "dense_lane_completions"],
        unsupported_modes: &["gemma4_lane", "diffusion_gemma_lane"],
    },
    ApiConformanceCase {
        id: "sampling_top_p",
        status: SupportStatus::SupportedCurrentGate,
        notes: "top_p is applied on the dense and runnable chat lanes, streaming and non-streaming. Must be finite and in (0, 1]; anything else is a typed 400. Only meaningful with temperature above 0. The gemma4 and diffusion-gemma lanes ignore sampling and always decode greedily, so neither the typed 400s nor the parameter itself applies there.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "dense_lane_completions"],
        unsupported_modes: &["gemma4_lane", "diffusion_gemma_lane"],
    },
    ApiConformanceCase {
        id: "sampling_top_k",
        status: SupportStatus::SupportedCurrentGate,
        notes: "top_k is applied on the dense and runnable chat lanes, streaming and non-streaming. top_k of 1 keeps only the highest-scoring token. top_k of 0 is a typed 400 invalid_sampling_parameter; a negative value cannot deserialize into an unsigned field and is a typed 400 malformed JSON instead. Only meaningful with temperature above 0. The gemma4 and diffusion-gemma lanes ignore sampling and always decode greedily, so neither the typed 400s nor the parameter itself applies there.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "dense_lane_completions"],
        unsupported_modes: &["gemma4_lane", "diffusion_gemma_lane"],
    },
    ApiConformanceCase {
        id: "sampling_min_p",
        status: SupportStatus::SupportedCurrentGate,
        notes: "min_p is applied on the dense and runnable chat lanes, streaming and non-streaming. Must be finite and in [0, 1]; anything else is a typed 400. Only meaningful with temperature above 0. The gemma4 and diffusion-gemma lanes ignore sampling and always decode greedily, so neither the typed 400s nor the parameter itself applies there.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "dense_lane_completions"],
        unsupported_modes: &["gemma4_lane", "diffusion_gemma_lane"],
    },
    ApiConformanceCase {
        id: "sampling_seed",
        status: SupportStatus::SupportedCurrentGate,
        notes: "seed makes sampled generation reproducible on the dense and runnable chat lanes: the same seed and request return the same text. Two different seeds are NOT guaranteed to differ — a peaked step, or top_k of 1, selects the same token for every seed. Without a seed, sampled decoding is still deterministic per request. The two lanes do not share a random stream, so a seed reproduces a reply within a lane, not across them. A seed has no observable effect at temperature 0, where decoding is greedy. The gemma4 and diffusion-gemma lanes ignore sampling and always decode greedily, so neither the typed 400s nor the parameter itself applies there.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "dense_lane_completions"],
        unsupported_modes: &["gemma4_lane", "diffusion_gemma_lane"],
    },
    ApiConformanceCase {
        id: "sampling_stop",
        status: SupportStatus::SupportedCurrentGate,
        notes: "Up to 4 stop strings truncate the reply on the dense and runnable chat lanes, streaming and non-streaming; the response reports finish_reason stop. A stop string may span or split tokens, so the token that completes it is retained in the token ids and in usage.completion_tokens while only the text is cut. An empty array, an empty string, or more than 4 sequences is a typed 400. Deciding a stop over decoded text costs a detokenization of the generated prefix per step, so a request carrying stop is quadratic in reply length; a request without stop short-circuits and is unaffected. The gemma4 and diffusion-gemma lanes do not read stop.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "dense_lane_completions"],
        unsupported_modes: &["gemma4_lane", "diffusion_gemma_lane"],
    },
    ApiConformanceCase {
        id: "sampling_repeat_penalty",
        status: SupportStatus::SupportedCurrentGate,
        notes: "repeat_penalty, presence_penalty and frequency_penalty are applied on the dense and runnable chat lanes over the penalty window. Their effect is only observable on a generation with something to penalize; a short reply may be unchanged. Parity evidence for supported rows is greedy and unpenalized. The gemma4 and diffusion-gemma lanes ignore sampling and always decode greedily, so neither the typed 400s nor the parameter itself applies there.",
        routes: &[post("/v1/chat/completions"), post("/v1/completions")],
        supported_modes: &["chat_streaming", "chat_nonstreaming", "dense_lane_completions"],
        unsupported_modes: &["gemma4_lane", "diffusion_gemma_lane"],
    },
