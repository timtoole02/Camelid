...
/// unusable total-memory value, in which case callers must use a bounded fallback
/// rather than deriving an allocation from it. Zero available bytes is a valid
/// pressure signal and must remain visible to allocation admission.
