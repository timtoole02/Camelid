...
- the default-off `CAMELID_X86_Q8_OUTPUT_VNNI_DECODE` logits route is explicitly rejected on fresh Ubuntu Linux x86_64 same-host evidence: PR #127 head `74d71c4` preserved one-token parity and hit the intended logits routes, but regressed Camelid TTFT from `3329.59 ms` on the retained `x86_output_decode_owner` baseline to `4510.49 ms`, with the rawptr variant still slower at `4429.63 ms`; keep it default-off and do not retain it as a throughput slice in its current form.
...
- `reports/ubuntu-x86-q8/pr127-output-vnni-rejection-20260524T110827Z.md` (fresh Ubuntu Linux x86_64 same-host rejection note for PR #127: the output VNNI logits route preserved one-token parity and hit the intended route labels, but regressed Camelid TTFT by about `35.5%` versus the retained `main` output decode-owner baseline, with the rawptr variant still about `33.0%` slower)
