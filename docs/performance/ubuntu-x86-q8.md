...
- ExecutionPlan now treats the x86 attention Q/K/V, attention-output, output, FFN gate/up/down decode-consumer, attention Q/K/V prefill-consumer, FFN gate/up single-owner, packed-rows4 FFN-down matmul, packed-rows4 FFN gate/up matmul, packed-rows4 attention-Q/K/V matmul, packed-rows4 attention-output matmul, packed-rows4 output matmul, FFN-down GEMM4, packed-rows4 AVX2-dot/hoist/serial-decode, parallel-input-quantize, and packed-rows4 chunk-size tuning flags as managed default-off/cleared knobs, so appliance planning clears stale owner experiments instead of inheriting them accidentally.
...
- `qa/evidence-bundles/llamacpp-q8-cpu-re-20260514T1200Z/artifacts/cron-95495a91-20260519T0148Z-x86-q8-stale-env-guard/README.md` (retained default-off execution-plan guard for additional stale x86 Q8 experimental env leakage; control-plane hygiene only, not throughput/parity/support evidence)
