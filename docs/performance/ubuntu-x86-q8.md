...
- the default-off `CAMELID_X86_Q8_OUTPUT_AMX_PREFILL` logits prefill helper is explicitly rejected on fresh Ubuntu Linux x86_64 same-host evidence: one-token parity stayed exact and the focused Linux x86 AMX parity test passed, but enabling `CAMELID_X86_Q8_AMX_REPACK=on CAMELID_X86_Q8_OUTPUT_AMX_PREFILL=on` regressed Camelid TTFT from `3283.36 ms` on the retained packed-rows4 baseline to `4024.04 ms`, with backend first content worsening from `2798.67 ms` to `3523.67 ms`; keep it default-off and require a distinct telemetry label plus a new same-host win before retention.
...
- `reports/ubuntu-x86-q8/output-amx-prefill-rejection-20260524T141517Z.md` (fresh Ubuntu Linux x86_64 same-host rejection note for the default-off output AMX prefill helper: the focused AMX parity test passed and one-token output stayed exact, but enabling the AMX prefill lane regressed Camelid TTFT by about `22.6%` and backend first content by about `25.9%` versus the retained packed-rows4 baseline)
