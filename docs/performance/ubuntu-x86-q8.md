...
- `reports/ubuntu-x86-q8/planner-passthrough-knobs-benchmark-20260524T194018Z.md` (fresh Ubuntu Linux x86_64 same-host Llama 3.2 3B Q8_0 benchmark for `13e7f94`: exact one-token parity stayed `C`, the FFN-down route mix stayed stable, and the candidate beat current `main` on all measured runs, but the win remains bounded default-off evidence only and llama.cpp is still much faster)
