...
- `CAMELID_X86_Q8_PACKED_ROWS4_AVX512VNNI_DPWSSD_DOT=on` is a default-off Ubuntu x86_64 developer experiment that uses an AVX512VNNI signed-dot rows4/I8 block kernel inside packed-runtime dot paths on hosts with AVX2, AVX512F, AVX512BW, and AVX512VNNI. It falls back unless the explicit flag and runtime CPU feature checks pass. Current evidence retains it only for the bounded same-host Llama 3.2 3B Q8_0 lane.
