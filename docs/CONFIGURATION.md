...
- `CAMELID_CUDA_RESIDENT_PREFILL_BATCHED` overrides the resident CUDA prefill policy. Q8_0, Q4_K, and Q6_K use batched prefill by default. The Qwen3-4B Q4_K_M Windows/SM86 promotion measured 85.43 prompt tok/s batched versus 22.72 serial at 298 tokens and 52.16 versus 21.98 at 858 tokens. Set it to `0`, `false`, or `off` to force serial prefill for diagnosis, or `1`, `true`, or `on` to force the batched choice.
- `CAMELID_CUDA_Q6K_DP4A` controls the default-on Q6_K prefill tile with cooperative 6-bit unpack, signed `dp4a` pairs, and register-resident accumulators. Set it to `0`, `false`, or `off` to A/B the older scalar/shared-accumulator wide tile. Kernel parity is bit-exact at 1, 2, 8, and 32 tokens; the representative Windows SM86 projection benchmark improved from 62.953 ms to 24.042 ms. The whole-model and kernel receipt is `qa/perf/qwen3-4b-q4km-windows-q6k-dp4a-20260813.json`.
- `CAMELID_CUDA_KQUANT_BATCH_TOKENS` selects the requested Q4_K/Q6_K CUDA prefill tile size from `1` through `64`. Default: `32`; the runtime clamps it to the model dimensions and shared-memory/occupancy budget.
