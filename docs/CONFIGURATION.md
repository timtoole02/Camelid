...
- `CAMELID_X86_Q8_FFN_DOWN_VNNI_DECODE_GROUP_CHUNKING=on` is a default-off Ubuntu/Linux x86_64 developer experiment for the existing FFN-down VNNI raw-pointer decode route. It chunks 64-output VNNI decode groups into coarser Rayon jobs and only affects the already gated `CAMELID_X86_Q8_FFN_DOWN_VNNI_DECODE_RAWPTR` helper; leave it unset outside focused evidence runs.
