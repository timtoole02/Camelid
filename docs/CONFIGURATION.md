...
- `CAMELID_X86_Q8_FFN_DOWN_DECODE_CONSUMER=on` is a default-off Ubuntu x86_64 developer experiment that lets one-row dense FFN-down projection consume backend-owned packed `blk.*.ffn_down.weight` storage directly. `CAMELID_X86_Q8_FFN_DOWN_DECODE_GROUP_CHUNKING=on` is the narrower default-off follow-on that reduces decode output-group task fan-out for that same route, and `CAMELID_X86_Q8_FFN_DOWN_DECODE_GROUPS_PER_CHUNK` tunes its chunk size when the gate is enabled. The execution planner clears stale values unless a future slice explicitly owns and validates that consumer gate for the run.
