...
- `CAMELID_X86_Q8_FFN_DECODE_CHAIN=on` is a default-off Ubuntu x86_64 developer experiment that keeps the one-row FFN gate/up activation and FFN-down decode steps inside a single chained Q8 flow. It still depends on the existing FFN gate/up and FFN-down decode-consumer component gates, and denial telemetry now reports when either dependency is missing instead of silently disappearing into fallback.
