...
- `CAMELID_X86_Q8_FFN_DOWN_DECODE_CONSUMER=on` is a default-off Ubuntu x86_64 developer experiment that lets one-row dense FFN-down projection consume backend-owned packed `blk.*.ffn_down.weight` storage directly. `CAMELID_X86_Q8_FFN_DOWN_DECODE_GROUP_CHUNKING=on` is a narrower default-off scheduler follow-on for that same route, reducing Rayon task fan-out by chunking output groups only after the decode-consumer guard is active. The execution planner manages both flags, clears stale values by default, and preserves them only when an explicit experimental opt-in owns the run.
