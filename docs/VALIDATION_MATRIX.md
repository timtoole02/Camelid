...
| CUDA batching or resident-arena changes | Standard backend gate + `node scripts/check-cuda-prefill-parity-gate.mjs` + focused `phase7_`/`phase8_`/`phase9_` tests | Exact-model batch-eight regression; capacity-two A-B-A-B physical run; active/idle unload; packaged-artifact smoke; `npm run smoke:arena` and `npm run smoke:arena-browser` | Browser matrix must cover capacity two, default one, omitted arena field, one-model empty state, active residency, long IDs on mobile, and runtime-memory failure. Keep defaults off and support claims exact-envelope only. |
