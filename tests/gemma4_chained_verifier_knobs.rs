...
#[test]
fn test_gemma4_chained_verifier_knobs() {
    ...
    // Knobs (env): KNOB_SLOTS (slots/layer, default 20), KNOB_CACHE_MIB (host expert cache, default 2900),
    // KNOB_TOPN (prefetch top-N, default 14), KNOB_ROUNDS (default 10), KNOB_SKIP_PREFETCH=1, KNOB_SKIP_ISOLATE=1
    let knob = |k: &str, d: usize| std::env::var(k).ok().and_then(|v| v.parse::<usize>().ok()).unwrap_or(d);
    let knob_slots = knob("KNOB_SLOTS", 20);
    let knob_cache_mib = knob("KNOB_CACHE_MIB", 2900);
    let knob_topn = knob("KNOB_TOPN", 14);
    let knob_rounds = knob("KNOB_ROUNDS", 10);
    let knob_skip_prefetch = knob("KNOB_SKIP_PREFETCH", 0) == 1;
    let knob_skip_isolate = knob("KNOB_SKIP_ISOLATE", 0) == 1;
    std::env::set_var("CAMELID_GEMMA4_GHOST_METAL_SLOTS", "1");
    std::env::set_var("CAMELID_GEMMA4_GHOST_METAL_SLOTS_FAST", "1");
    std::env::set_var("CAMELID_GEMMA4_GHOST_METAL_SLOTS_PER_LAYER", knob_slots.to_string());
    std::env::set_var("CAMELID_GEMMA4_GHOST_METAL", "1");
    std::env::set_var("CAMELID_GEMMA4_GHOST_METAL_STATS", "1");
    println!("KNOBS: slots/layer={knob_slots} cache_mib={knob_cache_mib} top_n={knob_topn} rounds={knob_rounds} skip_prefetch={knob_skip_prefetch} skip_isolate={knob_skip_isolate}");
    for (k, v) in std::env::vars() { if k.starts_with("CAMELID_GEMMA4") { println!("ENV {k}={v}"); } }
    ...
    let initial_logits = runtime.prefill_tokens(&prompt_tokens, &mut kc, &mut vc, 0).expect("prefill");
    ...
    for _ in 0..16 {
        ...
        let tok = cur_logits
            ...
            .unwrap();
        draft_pool.push(tok);
        cur_logits = runtime.step(tok, cur_pos, &mut temp_kc, &mut temp_vc).expect("step");
        cur_pos += 1;
        roll_times.push(t_one.elapsed().as_secs_f64() * 1000.0);
    }
    println!("K=1 step() rollout: 16 tokens in {:.1} ms; per-token ms = {:?}", t_roll.elapsed().as_secs_f64()*1000.0, roll_times.iter().map(|v| (v*10.0).round()/10.0).collect::<Vec<_>>());
    ...
    let prefetched_count = if knob_skip_prefetch { 0 } else { runtime.prefetch_round_wide_chunk_top_n(candidate_chunk, top_n).unwrap_or(0) };
    let prefetch_ms = t_prefetch_start.elapsed().as_secs_f64() * 1000.0;
    println!("Prefetch completed: {:.2} ms ({} experts prefetched)\n", prefetch_ms, prefetched_count);

    // Warmup (discarded cold round)
    let mut round_kc = kc.clone();
    let mut round_vc = vc.clone();
    let _ = runtime.step_chunk_profiled(candidate_chunk, start_pos, &mut round_kc, &mut round_vc).expect("warmup");
    ...
    for (i, &tok) in candidate_chunk.iter().enumerate() {
        let logits = runtime.step(tok, start_pos + i, &mut seq_kc, &mut seq_vc).expect("k1 step");
        k1_logits.push(logits);
    }
    runtime.rollback_sequence(start_pos);
    let mut cmp_kc = kc.clone();
    let mut cmp_vc = vc.clone();
    let (k8_logits, _) = runtime.step_chunk_profiled(candidate_chunk, start_pos, &mut cmp_kc, &mut cmp_vc).expect("k8 isolate");
    runtime.rollback_sequence(start_pos);
    ...
    for i in 0..k {
        ...
        let expected = if i + 1 < k { candidate_chunk[i + 1] } else { u32::MAX };
        let k8_abs: f32 = k8_logits[i].iter().map(|v| v.abs()).sum();
        let k1_abs: f32 = k1_logits[i].iter().map(|v| v.abs()).sum();
        let k8_max = k8_logits[i].iter().copied().fold(f32::NEG_INFINITY, f32::max);
        let k1_max = k1_logits[i].iter().copied().fold(f32::NEG_INFINITY, f32::max);
    ...
    fn vm_stat_compressor_mb() -> f64 {
        ...
        let pages = out.lines().find_map(|l| {
            if l.contains("occupied by compressor") || l.contains("Pages compressor") {
                l.split(':')
                    .nth(1)
                    .and_then(|s| s.trim().trim_end_matches('.').replace(',', "").parse::<f64>().ok())
            } else {
                None
            }
        });
    ...
    fn mac_memory_report() -> (f64, f64, String) {
        ...
        let swap_used = swap_txt
            ...
            .and_then(|s| s.trim().parse::<f64>().ok())
            .unwrap_or(0.0);
        (compressed_mb, swap_used, format!("{} | {}", pressure_line, swap_txt.trim()))
    }
    ...
    println!("  GPU stage timestamps via queued per-stage CBs (GPUStartTime; no extra host waits).");
    ...
    for round in 0..knob_rounds {
        ...
        let (rows, prof) = runtime.step_chunk_profiled(candidate_chunk, start_pos, &mut test_kc, &mut test_vc).expect("verify");
    ...
    // K=1 profiled steps through the chained lane (what real decode pays per token).
    {
        let mut k1_kc = kc.clone();
        ...
        for i in 0..6usize {
            ...
            let t = Instant::now();
            let (_rows, p) = runtime.step_chunk_profiled(&[tok], start_pos + i, &mut k1_kc, &mut k1_vc).expect("k1 profiled");
            let wall = t.elapsed().as_secs_f64() * 1000.0;
            println!(
                "  k1[{i}] wall={wall:.1}ms gpu_busy={:.1} slot_wait={:.1} final_wait={:.1} filler_cpu(host)={:.1} setup={:.1} encode={:.1} nvme_ms={:.1} nvme_mb={:.1} demand={} head={:.1} other={:.1} chained_ok={} unique={}",
                p.chained_gpu_busy_ms, p.chained_slot_wait_ms, p.chained_final_wait_ms,
                (p.cp_cache_slot_lookup_ms - p.chained_setup_ms).max(0.0), p.chained_setup_ms,
                p.cp_command_encoding_ms, p.physical_ssd_reads_ms, p.physical_nvme_mb, p.prefetch_late_count,
                p.cp_output_head_ms, p.cp_other_ms, p.gpu_chained_round_ok, p.unique_experts_sum
            );
        }
        runtime.rollback_sequence(start_pos);
    }
    ...
    println!("==========================================================================================================");
    println!("  true outer round latency (verifier wall): {:8.2} ms", avg_wall);
    println!("  verifier latency:                         {:8.2} ms", avg_wall);
    println!("  committed tokens/round:                   {:8.2}  (K=8 candidates; win metric is commits)", avg_commit);
    println!("  emitted tok/s (commits/wall):             {:8.2}", avg_tok_s);
    let partition_sum = avg_encode + avg_filler + avg_setup + avg_prefetch + avg_sync + avg_other + avg_head;
    let true_gap = (avg_wall - partition_sum).max(0.0);
    println!("  --- summing wall partition ---");
    println!("  command encoding (CPU):                   {:8.2} ms", avg_encode);
    println!("  CPU slot-table setup:                     {:8.2} ms", avg_setup);
    println!("  sync exposed (slot+final wait):           {:8.2} ms", avg_sync);
    println!("    nested slot_wait:                       {:8.2} ms", avg_slot);
    println!("    nested final_wait:                      {:8.2} ms", avg_final);
    println!("  embed/upload/rope/download:               {:8.2} ms", avg_other);
    println!("  output head:                              {:8.2} ms", avg_head);
    println!("  unexplained gap (lock/debug/eprint):      {:8.2} ms", true_gap);
    println!("  SUM of partition:                         {:8.2} ms", partition_sum + true_gap);
    println!("  wall-clock:                               {:8.2} ms", avg_wall);
    println!("  --- nested inside sync / background ---");
    println!("  pure Metal GPU critical path (GPU busy):  {:8.2} ms", avg_gpu);
    println!("  wait idle / driver (sync - gpu_busy):     {:8.2} ms", (avg_sync - avg_gpu).max(0.0));
    println!("  GPU stage timestamps (nested in gpu_busy; no extra waits):");
    println!("    QKV/O:                                  {:8.2} ms", avg(|r| r.qkv_o));
    println!("    attention:                              {:8.2} ms", avg(|r| r.attn));
    println!("    router:                                 {:8.2} ms", avg(|r| r.router));
    println!("    shared MLP:                             {:8.2} ms", avg(|r| r.shared));
    println!("    GateUp:                                 {:8.2} ms", avg(|r| r.gateup));
    println!("    Down:                                   {:8.2} ms", avg(|r| r.down));
    println!("    residual/norm/quant:                    {:8.2} ms", avg(|r| r.resid));
    let avg_stage = avg(|r| r.qkv_o + r.attn + r.router + r.shared + r.gateup + r.down + r.resid);
    println!("    SUM of GPU stages:                      {:8.2} ms", avg_stage);
    println!("  background NVMe throughput (overlapped):  {:8.2} ms ({:.2} MB)", avg_nvme, avg(|r| r.nvme_mb));
    println!("  late prefetches (demand slot loads/round):{:8.2}", avg(|r| r.late as f64));
    println!("  memory pressure (process RSS):            {:8.0} MiB", avg(|r| r.rss_mb));
    println!("  gpu_chained_round_ok every round:         {}", if all_chained { "true (fallback=0)" } else { "FALSE — see per-round" });
    println!("  parity every round:                       {} (Metal draft vs Metal verifier argmax)", if all_parity { "PASS" } else { "FAIL" });
    println!("  unique experts (sum/max over layers):     {:.0} / {:.0}", avg(|r| r.unique_sum as f64), avg(|r| r.unique_max as f64));
    println!("  expert waves (sum/max over layers):       {:.0} / {:.0}", avg(|r| r.waves_sum as f64), avg(|r| r.waves_max as f64));
    println!("  selected_experts_dropped:                 {:.0}", avg(|r| r.dropped as f64));
    println!("  missing_expert_failclose:                 {:.0}", avg(|r| r.failclose as f64));
    println!("  slot_capacity_overflow:                   {:.0}", avg(|r| r.overflow as f64));
    println!("  wave_load_ms:                             {:8.2}", avg(|r| r.wave_load));
    println!("  wave_gpu_ms:                              {:8.2}", avg(|r| r.wave_gpu));
    println!("  physical_nvme_mb:                         {:8.2}", avg(|r| r.nvme_mb));
    println!("==========================================================================================================");
    ...
    println!("#1 wall is '{}' — GPU #1 is '{}'.", ranked[0].0, gpu_ranked[0].0);
    ...
    let components: [(&str, u64, f64, u64, u64, f64); 8] = [
        ("GateUp", gate_logical, 1.0, act_in, gate_act_q8 + gate_act_scale, savg(|r| r.gateup)),
        ("Down", down_logical, down_est_dram as f64 / down_logical.max(1) as f64, gate_act_q8, act_in, savg(|r| r.down)),
        ("QKV/O", QKV_O, 1.0, act_in, K * 16 * 256 * 4, savg(|r| r.qkv_o)),
        ("Shared MLP", SHARED, 1.0, act_in, K * 2112 * 4, savg(|r| r.shared)),
        ("Head", HEAD, 1.0, act_in, 262_144 * 4 * K, savg(|r| r.head)),
        ("Attention", kv_logical_read, 1.0, K * 16 * 256 * 4, K * 16 * 256 * 4, savg(|r| r.attn)),
        ("Residual/norm", LAYERS * HIDDEN * 4 * 4, 1.0, act_in, act_in, savg(|r| r.resid)),
        ("Expert slot memcpy", slot_memcpy, 1.0, 0, 0, savg(|r| r.wave_load)),
    ];

    println!("==========================================================================================================");
    println!("BYTE LEDGER (K=8, unique={}, filled_pos={}, kv_cap={})", unique, filled, last.kv_capacity);
    ...
    for (name, logical, passes, act_r, act_w, ms) in components {
        let est = if name == "Down" { down_est_dram } else { (logical as f64 * passes) as u64 };
        let gbs = if ms > 0.0 { (est as f64 / 1e9) / (ms / 1000.0) } else { 0.0 };
        tot_logical += logical;
        tot_dram += est;
        println!(
    ...
    println!("---");
    println!("TOTAL logical weight+KV+slots:     {:>12}  ({:.3} GB)", tot_logical, tot_logical as f64 / 1e9);
    println!("MINIMUM mandatory (1-pass unique GateUp+Down + QKV + Shared + Head + Router + KV read):");
    println!("                                   {:>12}  ({:.3} GB)", mandatory, mandatory as f64 / 1e9);
    println!("ESTIMATED physical DRAM (Down rereads + 1-pass others + slot memcpy): {:>12}  ({:.3} GB)", tot_dram, tot_dram as f64 / 1e9);
    println!("GPU wall (steady 3..):             {:>8.2} ms", gpu_wall);
    println!("TRUE WALL (steady 3..):            {:>8.2} ms   ACTUAL tok/s {:.2}", wall, tok_s);
    println!("effective GB/s vs 120: GateUp {:.1}  Down {:.1}  QKV {:.1}  Head {:.1}",
        if savg(|r| r.gateup) > 0.0 { (gate_logical as f64 / 1e9) / (savg(|r| r.gateup) / 1000.0) } else { 0.0 },
        if savg(|r| r.down) > 0.0 { (down_est_dram as f64 / 1e9) / (savg(|r| r.down) / 1000.0) } else { 0.0 },
        if savg(|r| r.qkv_o) > 0.0 { (QKV_O as f64 / 1e9) / (savg(|r| r.qkv_o) / 1000.0) } else { 0.0 },
        if savg(|r| r.head) > 0.0 { (HEAD as f64 / 1e9) / (savg(|r| r.head) / 1000.0) } else { 0.0 },
    );
    println!("page-cache / file-backed: Head + QKV + Shared + Router are mmap/file-backed (may be cache-serviced, not extra DRAM alloc).");
    println!("expert slots resident: 24×30×{SLOT} = {:.2} GiB", (24.0 * 30.0 * SLOT as f64) / (1024.0 * 1024.0 * 1024.0));
    println!(
    ...
    println!("KV allocated: {:.2} GiB  (full-4096 would be {:.2} GiB, saved {:.2} GiB)",
    ...
    println!("wave-2: {} layers, {} overflow experts, memcpy {:.1} MiB",
        wave2_layers, wave2_experts, wave2_memcpy as f64 / (1024.0 * 1024.0));
    println!("Budgets at {:.0} GB/s: 100 tok/s=80ms≈9.60GB  75=106.67ms≈12.80GB  50=160ms≈19.20GB", PLANNING_BW_GBS);
    println!(
        "Mandatory {:.3} GB vs 50-tok budget 19.20 GB: {}",
        mandatory as f64 / 1e9,
        if mandatory as f64 / 1e9 > 19.20 { "EXCEEDS — 50 tok/s impossible without changing representation" } else { "fits 50 tok/s budget if kernels hit ~120 GB/s" }
    );
    println!(
        "Mandatory {:.3} GB vs 75-tok budget 12.80 GB: {}",
        mandatory as f64 / 1e9,
        if mandatory as f64 / 1e9 > 12.80 { "EXCEEDS — 75 tok/s needs fewer bytes or more than 120 GB/s" } else { "fits 75 tok/s budget if kernels hit ~120 GB/s" }
    );
    println!(
        "Mandatory {:.3} GB vs 100-tok budget 9.60 GB: {}",
        mandatory as f64 / 1e9,
        if mandatory as f64 / 1e9 > 9.60 { "EXCEEDS — 100 tok/s needs fewer bytes or more than 120 GB/s" } else { "fits 100 tok/s budget if kernels hit ~120 GB/s" }
    );
    println!("unique/layer: mean={:.1} p50={} p90={} p95={} p99={} max={}  (n=30)",
        mean_u, pct(50.0), pct(90.0), pct(95.0), pct(99.0), unions.last().copied().unwrap_or(0));
