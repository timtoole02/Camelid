...
- LFM2.5-2.6B `Q8_0` is a hash-pinned supported exact-row smoke lane, GPU-resident on Metal by default on Apple Silicon, with 96/96 greedy-token parity plus tokenizer/template gates.

Near-term objective: preserve every supported exact row across all support surfaces without broadening any of them, preserve LFM2.5's newly promoted hash-pinned envelope, close the active-validation lanes (Mixtral, Mistral Nemo, Qwen3 14B, DeepSeek R1 0528 Qwen3 8B, and the two distributed Gemma 4 rows) or keep them visibly unpromoted, clear the Phi-3 HOLD blockers before any Phi wording, and keep performance and GPU claims scoped to the exact row, host, and device they were measured on.
...
- Keep the two distributed Gemma 4 rows visibly active-validation; preserve LFM2.5's narrow hash-pinned exact-row smoke boundary.
...
- Close the active-validation set as exact-row lanes, never as family-wide claims: Mixtral 8x7B Instruct v0.1 Q8_0; Mistral Nemo Q4_K_M; Qwen3 14B Q4_K_M; DeepSeek R1 0528 Qwen3 8B Q4_K_M; and the two distributed Gemma 4 rows. LFM2.5-2.6B Q8_0 has left this set as a narrow hash-pinned supported exact-row smoke lane.
