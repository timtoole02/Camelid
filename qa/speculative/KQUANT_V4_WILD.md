...
path: the standard resident Llama prefill caller used by `bench-eagle3`
explicitly disallows V4 widths 9 through 16 so its normal 16-row chunks cannot
accidentally enter the verifier experiment. This is not a global guard for every
windowed or architecture-specific prefill surface. The kbench micro cases select
`q4kv4` / `q6kv4` (narrow) or `q4kv4w` /
