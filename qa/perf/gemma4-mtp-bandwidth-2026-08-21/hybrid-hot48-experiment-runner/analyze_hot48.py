#!/usr/bin/env python3
"""Fail-closed receipt analyzer for the uniform Hot48 48-token experiment."""
...
HOT48_STARTUP_PATTERN = re.compile(
    r"^\[gemma4-ghost-metal\] HYBRID HOT48 EXPERIMENT ACTIVE: .*"
    r"HYBRID_HOT_SLOTS=32 .*HYBRID_HOT48_EXPERIMENT=1, "
    r"layers=30 canonical_addressable=128/layer physical_hot=48/layer "
    r"hot_capacity_slots=1440 hot_capacity_bytes=4836556800 "
    r"mapped_cold_span_bytes=12897484800 overflow=0 victim=0 "
...
def _validate_hashes(run_dir: Path, intent: dict[str, Any]) -> None:
    ...
    if not isinstance(artifacts, dict) or set(artifacts) != required:
        raise ReceiptError("intent artifact hash manifest is incomplete or has extra entries")
    for label in sorted(required):
        ...
        if (
            ...
            raise ReceiptError(f"intent artifact {label} has an invalid identity")
...
def _validate_intent(run_dir: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or intent.get("benchmark") != "gemma4-uniform-hot48-experiment"
        ...
        raise ReceiptError("intent does not bind the exact Hot48 experiment")
...
def _validate_watchdog(run_dir: Path) -> dict[str, Any]:
...
def _validate_geometry(geometry: Any) -> None:
    ...
    if not isinstance(geometry, dict) or any(geometry.get(key) != value for key, value in expected.items()):
        raise ReceiptError("telemetry aggregate geometry is not exact uniform Hot48")
    ...
    for layer_index, layer in enumerate(per_layer):
        ...
        if not isinstance(layer, dict) or layer != expected_layer:
            raise ReceiptError(f"layer {layer_index} is not exact 48-hot/128-mapped geometry")
...
def _validate_log(
    ...
    if len(HOT48_STARTUP_PATTERN.findall(log)) != 1:
        raise ReceiptError("server log does not contain one exact uniform Hot48 admission")
...
def analyze(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "gates": {
            ...
            "uniform_48_by_30_geometry": True,
