...
| `H61-wide-qkv-staged` (source-history audit; no profile) | Exact wide-QKV SG4/TB8 activation staging duplicated the already-falsified runtime-width staged family in `9b28d3f9`; the tiled candidates, including SG4/TB8, measured 0.438--0.964x on M4, so H61 stopped before a build or Mini2 run |
| `H62-mtp-bf16-lattice-loads` | Default-off assistant query/probability BF16 lattice-load elimination: focused exact gates passed 5/5 plus two production oracle passes, but the ignored exact 44-draft microbenchmark saved only 3,087 us/request (55,186 us control versus 52,099 us candidate) across 218,767,360 eliminated rounds and 528 attention dispatches, below the 5,000 us gate; no-go with no Mini2 run, source and profile retained for reproducibility |
| `H63-terminal-cold-mma` (source/timing closure; no profile) | Default-off H55-only terminal K13/K14 staged-MMA GateUp with hot/K7 established fallback and unchanged two-command-buffer ordering: four focused exact/safety gates passed, but the production-histogram timing gate failed its first sparse one-hot round at 16,010 us versus 10,649 us established (paired delta -5,551 us); fail-fast no-go with no Mini2 run |
| `H64-async-two-wave-chunked-read` | Default-off H55-only adaptive 2–4-way positioned-read fanout for sparse authoritative `StageColdLaunch` batches; clean post-restart Mini2 H55/H64/H64/H55 measured 32.2237 tok/s control versus 32.0718 candidate, with 48/48 exact parity and zero swap throughout; verifier wall regressed 4.33 ms/request and round 0 regressed 6.82 ms, no-go |
| `H65-wide-down-mma-terms` (source/timing closure; no profile) | Exact 256-thread compacted integer-MMA Down probe: K13/K14/K16 raw-bit Metal parity passed, but the first 30-layer K14 production histogram took 278,369 us versus 28,540 us established (249,823 us regression, roughly 10x slower); zero swap, stopped before production wiring or ABBA |
| `H66-mtp-q4-head-row-pack` (source/timing closure; no profile) | Exact assistant head Row2/Row4 probe: both matched all 262,144 logits, but guarded 44-head paired timing regressed 2,131 us/request and 192 us/request respectively; zero swap, 212 MB peak child physical, 1.67 GB peak wired, stopped before production wiring or ABBA |
