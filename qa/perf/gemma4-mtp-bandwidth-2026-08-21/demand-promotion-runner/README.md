...
| `H64-async-two-wave-chunked-read` | Default-off H55-only adaptive 2–4-way positioned-read fanout for sparse authoritative `StageColdLaunch` batches; focused admission/attribution and existing exact reader gates pass, but Mini2 A/B is pending a clean zero-swap host because the first warmup was refused before child launch |
