...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let budgets = args[5].split(',').map(str::parse).collect::<Result<Vec<usize>, _>>()?;
    ...
    for budget in budgets {
        ...
        let control = runtime.generate_greedy_mtp12_ordered_q4(&mut assistant, &prompt, budget, 8)?;
        std::env::set_var("CAMELID_GEMMA4_MTP12_W8_PADDED_TAIL", "1");
        let candidate = runtime.generate_greedy_mtp12_ordered_q4(&mut assistant, &prompt, budget, 8)?;
        assert_eq!(candidate.token_ids, control.token_ids, "token parity at budget {budget}");
        assert_eq!(candidate.text, control.text, "text parity at budget {budget}");
        assert_eq!(candidate.stats.terminal_stop_token, control.stats.terminal_stop_token);
        assert!(candidate.token_ids.len() <= budget);
        let trace = &candidate.stats.trace;
        assert_eq!(candidate.stats.rounds, trace.len() as u64);
        assert_eq!(candidate.stats.target_verify_rows,
                   trace.iter().map(|r| r.physical_verify_width as u64).sum::<u64>());
        assert_eq!(candidate.stats.committed_input_rows,
                   trace.iter().map(|r| r.committed_input_rows as u64).sum::<u64>());
        assert_eq!(candidate.stats.accepted_drafts,
                   trace.iter().map(|r| r.accepted_drafts as u64).sum::<u64>());
        assert_eq!(candidate.stats.drafted,
                   trace.iter().map(|r| r.drafts.len() as u64).sum::<u64>());
        assert_eq!(candidate.stats.emitted_tokens, candidate.token_ids.len() as u64);
        let traced_output = trace.iter().flat_map(|r| r.emitted_token_ids.iter().copied()).collect::<Vec<_>>();
        ...
        if candidate.token_ids.len() != traced_output.len() {
            assert_eq!(trace.last().unwrap().next_anchor_token, candidate.token_ids.last().copied());
        }
        for round in trace {
            ...
            assert_eq!(round.padding_candidate_ids.len(), round.physical_verify_width - round.logical_verify_width);
            assert!(round.padding_candidate_ids.iter().all(|&id| id == round.anchor_token));
            assert_eq!(round.committed_input_rows, round.emitted_token_ids.len());
        }
        if let Some(first) = trace.first() {
            if available >= 8 && (5..=8).contains(&budget) {
                assert_eq!((first.logical_verify_width, first.physical_verify_width), (budget - 1, 8));
            } else if available < 8 || budget <= 4 {
                assert!(trace.iter().all(|r| r.padding_candidate_ids.is_empty()));
                assert_eq!(candidate.stats.target_verify_rows, control.stats.target_verify_rows);
    ...
    assert!(!require_stop || saw_stop, "stop case did not reach a natural terminal token; choose a shorter reply prompt");
